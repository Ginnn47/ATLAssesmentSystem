...
class AcademicPeriod(models.Model):
    ...
    class Meta:
        ordering = ["-is_active", "-academic_year", "semester", "name"]
...
class Student(models.Model):
    ...
    avatar_tone = models.CharField(max_length=120, blank=True)
    baseline_overall = models.CharField(max_length=20, blank=True)
    baseline_strength = models.CharField(max_length=120, blank=True)
    baseline_strength_value = models.CharField(max_length=20, blank=True)
    baseline_focus = models.CharField(max_length=120, blank=True)
    baseline_trend_value = models.CharField(max_length=20, blank=True)
    is_active = models.BooleanField(default=True)

    class Meta:
        ordering = ["school_class__code", "full_name"]
    ...
    def __str__(self):
        return f"{self.full_name} ({self.nis})"


class UserProfile(models.Model):
    user = models.OneToOneField(User, related_name="atl_profile", on_delete=models.CASCADE)
    nip = models.CharField(max_length=80, blank=True)
    role_label = models.CharField(max_length=120)
    role_group = models.CharField(max_length=80)
    status = models.CharField(max_length=40, default="Aktif")
    class_access = models.ManyToManyField(SchoolClass, related_name="user_profiles", blank=True)
    subject_access = models.ManyToManyField("Subject", related_name="user_profiles", blank=True)
    last_login_label = models.CharField(max_length=80, default="-")

    class Meta:
        ordering = ["role_group", "user__first_name", "user__username"]

    def __str__(self):
        name = self.user.get_full_name() or self.user.username
        return f"{name} - {self.role_label}"
...
class PairwiseScaleOption(models.Model):
    context = models.ForeignKey(LearningContext, related_name="pairwise_scale_options", on_delete=models.CASCADE)
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["context", "code"], name="unique_pairwise_scale_option_per_context")
        ]
...
class StudentAssessment(models.Model):
    student_id = models.CharField(max_length=80)
    student_record = models.ForeignKey(Student, related_name="context_assessments", on_delete=models.SET_NULL, blank=True, null=True)
    academic_period = models.ForeignKey(AcademicPeriod, related_name="student_assessments", on_delete=models.SET_NULL, blank=True, null=True)
